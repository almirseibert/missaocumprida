    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      {error ? (
        <div className="text-center py-16">
          ...
          <p className="text-lg font-medium text-gray-700">Erro ao carregar propostas</p>
          <p className="text-sm text-gray-500 mt-1">Verifique sua conexão e tente novamente.</p>
        </div>
      ) : proposals.length === 0 ? (
        <div className="text-center py-16">
          <p className="text-4xl mb-3">📋</p>
          <p className="text-lg font-medium text-gray-700">Nenhuma proposta enviada</p>
          <p className="text-sm text-gray-500 mt-1">Explore o feed e envie propostas para os pedidos disponíveis.</p>
          <Link
            href="/feed"
        ...
        <div className="space-y-3">
          {proposals.map((proposal) => (
            <Link
              ...
              className="flex items-start justify-between gap-3 p-4 bg-white rounded-2xl border border-gray-200 hover:border-brand-300 hover:shadow-sm transition-all"
            >
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-gray-900 truncate">
                  {proposal.order?.category?.icon} {proposal.order?.title || 'Pedido'}
                </p>
                <p className="text-xs text-gray-500 mt-1">{formatRelative(proposal.created_at)}</p>
                {proposal.message && (
                  <p className="text-sm text-gray-600 mt-1 truncate">{proposal.message}</p>
                )}
              </div>
